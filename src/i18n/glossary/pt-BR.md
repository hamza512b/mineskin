...
| English | pt-BR | Notes |
|---|---|---|
| **Reference** <br><sub>Panel title for the reference-image panel</sub> | Referência | Current: 'Referência' (reference.title). Singular noun for the panel heading; the plural 'Referências' is used only when the string genuinely refers to several images (reference.allColors, reference.allReferences). |
...
| **Palette** <br><sub>Colors extracted from a reference image (see also Color › Palette)</sub> | Paleta | Same canonical term as colorPicker.paletteTab. The reference strings phrase it as 'Cores desta imagem' / 'Cores de todas as referências' (reference.imageColors / allColors) because the source says "Colors in …", not "Palette" — keep 'Paleta' for any string that literally says Palette. |
